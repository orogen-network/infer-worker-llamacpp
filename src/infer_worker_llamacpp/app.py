"""FastAPI worker app — llama.cpp edge variant."""
...
from infer_worker_llamacpp.engine import Engine, MockLlamaCppEngine
from infer_worker_llamacpp.heartbeat import HeartbeatPusher, build_heartbeat
...
def build_app(
    ...
    engine_impl: Engine = engine or MockLlamaCppEngine(model_id=config.model_id)
    state: dict[str, Any] = {
        "requests_total": 0,
        "active_requests": 0,
        "last_receipt": None,
    ...
    app = FastAPI(title="infer-worker-llamacpp", version="0.1.0", lifespan=lifespan)
    ...
    async def chat_completions(req: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            receipt = Receipt(
                ...
                kv_metadata=KvMetadata(cache_hit=False, kv_blocks_used=1),
