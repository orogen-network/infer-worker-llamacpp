...
- If neither `model_path` nor `llamacpp_model_file` is set / exists, this is
  treated as a Mock engine — log a warning and skip (the mock has no
  on-disk weights).
...
def _resolve_model_path(config: Any) -> str | None:
    """llama.cpp stores its GGUF path on `llamacpp_model_file`; allow either
    that or a generic `model_path` for forward-compatibility."""
    for attr in ("model_path", "llamacpp_model_file"):
...
def verify_weights(config: Any) -> None:
